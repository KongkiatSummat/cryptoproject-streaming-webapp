  ...
  export default function Home() {
    ...
    return (
      <main className="min-h-screen bg-gray-950 text-white">
        ...
        {/* <SubscriptionBanner /> */}
        <SubscriptionBanner onSubscribed={() => setRefreshKey(k => k + 1)} />
        ...
        <div className="px-6 py-6 space-y-10">
          {loading ? (
            ...
            <>
              ...
              <MovieRow key={`all-${refreshKey}`} title="🔥 หนังทั้งหมด" movies={movies} onWatch={setSelectedMovie} />

              {genres.map(genre => (
                <MovieRow
                  key={`${genre}-${refreshKey}`}
                  title={`🎭 ${genre}`}
